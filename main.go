...
func main() {
	// 1. Konfigurasi environment (.env dipanggil di sini sekali)
	config.LoadEnv()

	// 2. Koneksi basis data — pool dipakai bersama oleh seluruh repository.
	...
	// 3. Perakitan dependency: pool → repository → handler.
	//    Handler hanya menerima INTERFACE, bukan struct konkret.
	...
	// /health ikut memeriksa kondisi basis data.
	// Sukses → 200. DB tidak dapat dihubungi → 503 Service Unavailable.
