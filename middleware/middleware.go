...
func Register(app *fiber.App, logger *slog.Logger) {
    app.Use(requestid.New()) //1. beri setiap 1 unik id
    app.Use(recover.New()) //2. tangkap panic agar server tidak mati
    app.Use(helmet.New()) //3. pasang header base security
    app.Use(cors.New()) //4. atur cross-origin resources sharing
    app.Use(RequestLogger(logger)) //catat setiap request
}
