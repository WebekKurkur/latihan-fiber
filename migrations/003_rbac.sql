-- Role yang diakui oleh aplikasi.
...
-- Tindakan yang dapat diberikan kepada role.
CREATE TABLE IF NOT EXISTS permissions (
	name        VARCHAR(50)  PRIMARY KEY,
	description VARCHAR(150) NOT NULL
);

-- Pemetaan many-to-many antara role dan permission.
...
-- Data lama mungkin berisi role yang belum dikenal sebelum foreign key dibuat.
UPDATE students
SET role = 'user'
WHERE role NOT IN (SELECT name FROM roles);

ALTER TABLE students
	DROP CONSTRAINT IF EXISTS students_role_fkey;
